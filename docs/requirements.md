...
| FR-1-9 | 取得失敗時は初回試行の後 `RETRY_COUNT` 回追加で再試行する（例: `RETRY_COUNT=2` なら合計3回試行）。429 の場合は指数バックオフを適用する |
...
| ID | 要件 |
|---|---|
| FR-8-1 | スリープ復帰後にキャッシュが `SLEEP_STALE_MINUTES` 分超の場合、`refresh.sh all` で即時更新する |
...
| FR-8-3 | `refresh.sh` 起動時にキャッシュ timestamp を確認し、`SLEEP_STALE_MINUTES` 超であれば強制更新する（通常の1分定期取得と同一エントリポイントで対応） |
...
| `RETRY_COUNT` | `2` | 追加リトライ回数（初回含む合計 `RETRY_COUNT + 1` 回、429 は指数バックオフ） |
...
| `SLEEP_STALE_MINUTES` | `5` | スリープ復帰後にこの分数超のキャッシュを即時更新（FR-8-1） |
...
- **Claude usage API**: 非公式エンドポイント（`api.anthropic.com/api/oauth/usage`）のため仕様変更リスクあり。公式の制限値は非公開。429 時は指数バックオフで対処し、デフォルト1分間隔が安全かは実運用で確認（U8）
